...
test.describe('Blog Post Features', () => {
  ...
  test('table of contents functionality', async ({ page }) => {
    ...
    for (const link of postLinks.slice(0, 5)) { // Test first 5 posts
      ...
      if (href) {
        ...
        if (await toc.isVisible() || await tocInside.isVisible() || await tocOutside.isVisible()) {
  ...
  test('social sharing buttons work', async ({ page }) => {
    ...
    if (await shareButtons.isVisible()) {
      await expect(shareButtons.locator('[data-testid="share-twitter"]')).toBeVisible()
      await expect(shareButtons.locator('[data-testid="share-facebook"]')).toBeVisible()
      await expect(shareButtons.locator('[data-testid="share-linkedin"]')).toBeVisible()
    }
  })
  ...
  test('post content is properly formatted', async ({ page }) => {
    ...
    if (paragraphCount > 1) {
      ...
      const marginBottom = await firstP.evaluate((el) => getComputedStyle(el).marginBottom)
