...
export default function CommandPalette({ allPosts, isOpen, onOpenChange }: CommandPaletteProps) {
  ...
  const filteredPosts = searchValue.length >= 2 
    ? allPosts.filter(post => {
        ...
        const categoryMatch = post.frontMatter.category.toLowerCase().includes(searchLower)
  ...
  const allTags = Array.from(new Set(allPosts.flatMap(post => post.frontMatter.tags)))
    .sort((a, b) => {
      // Sort by frequency
      const aCount = allPosts.filter(post => post.frontMatter.tags.includes(a)).length
      const bCount = allPosts.filter(post => post.frontMatter.tags.includes(b)).length
          ...
          <CommandGroup heading={`검색 결과 (${filteredPosts.length}개)`}>
            {filteredPosts.map((post) => (
              ...
              >
                ...
                <Fa icon={faArrowRight} className="text-xs text-muted-foreground mt-1" />
              </CommandItem>
            ))}
          </CommandGroup>
        )}
          ...
          <>
            <CommandGroup heading="최근 검색어">
              {recentSearches.map((search, index) => (
                ...
                >
                  <Fa icon={faHistory} className="text-sm text-muted-foreground" />
            ...
            <CommandGroup heading="인기 포스트">
              {popularPosts.map((post) => (
                <CommandItem
                  ...
                  className="flex items-start gap-3 p-3 cursor-pointer"
                >
                  <Fa icon={faStar} className="text-sm text-yellow-500 mt-0.5" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-medium text-sm truncate">
                        {post.frontMatter.title}
          ...
          <CommandGroup heading="인기 태그">
            {allTags.map((tag) => (
              ...
              >
                ...
                <Badge variant="secondary" className="text-xs">
                  {allPosts.filter(post => post.frontMatter.tags.includes(tag)).length}
                </Badge>
              </CommandItem>
            ))}
          </CommandGroup>
